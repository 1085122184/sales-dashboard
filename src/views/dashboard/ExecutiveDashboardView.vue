...
// ==========================================
const totalActualAmount = computed(() => rawData.reduce((s, d) => s + d.amount, 0))
const totalBudgetAmount = computed(() => rawData.reduce((s, d) => s + d.amountBudget, 0))
const totalActualVolume = computed(() => rawData.reduce((s, d) => s + d.volume, 0))
const totalBudgetVolume = computed(() => rawData.reduce((s, d) => s + d.volumeBudget, 0))

const globalMetrics = computed(() => {
  const totalVol = totalActualVolume.value
  const totalVolBudget = totalBudgetVolume.value
  const totalAmt = totalActualAmount.value
  const totalAmtBudget = totalBudgetAmount.value
...
@media (max-width: 1023px) {
  .exec-dashboard { padding: 20px 24px 40px; }
  .header-titles h1 { font-size: 24px; }
  .subtitle { font-size: 13px; }
  
  /* 指标卡改为 2 列 */
  .global-metrics { grid-template-columns: 1fr 1fr; gap: 20px; }
  .metric-card { padding: 20px; }
  .mc-value { font-size: 30px; }
  
  /* 调查区保持双栏但缩小间距 */
  .investigation-zone { gap: 20px; }
  .panel { padding: 20px; }
...
@media (max-width: 767px) {
  .exec-dashboard { padding: 16px 12px 32px; }
  .header-titles h1 { font-size: 20px; }
  .subtitle { font-size: 12px; }
  
  /* 指标卡改为单列 */
  .global-metrics { grid-template-columns: 1fr; gap: 16px; margin-bottom: 16px; }
  .metric-card { padding: 16px; border-radius: 12px; }
  .mc-title { font-size: 14px; }
  .mc-value { font-size: 26px; margin-bottom: 12px; }
  .mc-desc { font-size: 12px; flex-direction: column; gap: 4px; }
  .progress-track { height: 6px; margin-bottom: 10px; }
  
  /* 调查区改为单列堆叠 */
  .investigation-zone { grid-template-columns: 1fr; gap: 16px; margin-bottom: 16px; }
  .panel { padding: 16px 12px; border-radius: 12px; }
  .panel-header { flex-direction: column; gap: 12px; align-items: flex-start; margin-bottom: 16px; }
  .panel-header h2 { font-size: 16px; }
  .panel-sub { font-size: 12px; }
  
  /* 产品选择器全宽 */
  .product-select { width: 100%; font-size: 14px; padding: 6px 12px; }
  .scatter-chart { height: 260px; }
  
  /* 排行榜移动端优化 */
  .list-title { font-size: 14px; margin-bottom: 12px; }
  .rank-item { padding: 10px 0; }
  .rank-name { font-size: 14px; }
  .rank-prod { font-size: 12px; }
  .rank-data { flex-direction: row; align-items: center; gap: 12px; }
  .rank-sub { font-size: 12px; }
  
  /* 表格移动端优化 */
  .data-table { font-size: 13px; min-width: 700px; }
  .data-table th { padding: 10px 12px; font-size: 12px; }
  .data-table td { padding: 12px; font-size: 13px; }
  .pill { font-size: 12px; padding: 3px 6px; }
}

/* 超小屏（<= 420px） */
@media (max-width: 420px) {
  .exec-dashboard { padding: 12px 8px 24px; }
  .header-titles h1 { font-size: 18px; }
  .mc-value { font-size: 22px; }
  .panel-header h2 { font-size: 15px; }
  .scatter-chart { height: 220px; }
