...
const emit = defineEmits<{ (e: 'select', uniqueId: string | null): void }>()
...
const sortKey    = ref<SortKey>('correlation')
const sortAsc    = ref(true)

function healthRating(p: SalesTrendProduct) {
  if (p.correlation >= 0.3  && p.volumeChange >= 0 && p.priceChange >= 0) return { label: '优', cls: 'he', rank: 0 }
  if (p.correlation >= -0.2 && (p.volumeChange >= 0 || p.priceChange >= 0))  return { label: '良', cls: 'hg', rank: 1 }
  if (p.correlation >= -0.6) return { label: '中', cls: 'hm', rank: 2 }
  return { label: '差', cls: 'hp', rank: 3 }
}

// 🌟 优化：一次遍历完成所有统计，避免重复遍历
const tabCounts = computed(() => {
  let all = 0, poor = 0, good = 0
  for (const p of props.data) {
    all++
    const rank = healthRating(p).rank
    if (rank >= 2) poor++
    if (rank <= 1) good++
  }
  return { all, poor, good }
})
...
const processedData = computed(() => {
  ...
  list.sort((a, b) => {
    // 零销量特殊处理：如果任一产品的销量为0，则排到后面
    const aIsZero = a.latestVolume === 0
    const bIsZero = b.latestVolume === 0
...
function cc(v: number) { 
  if (Math.abs(v) < 0.001) return 'flat'  // 修正：当值接近0时返回'flat'
...
<template>
  <div class="panel">
    ...
    <!-- ── 预警横幅 ── -->
    <div v-if="tabCounts.poor > 0" class="alert-banner">
      ...
      <span class="banner-tag">{{ tabCounts.poor }} 个产品处于中/差状态</span>
    ...
    <div class="desktop-only">
      <div class="thead-wrap">
        <table class="base-table">
          <colgroup>
            <col style="width:27%"><col style="width:13%"><col style="width:11%">
            <col style="width:14%"><col style="width:11%"><col style="width:13%"><col style="width:11%">
          </colgroup>
          <thead>
            <tr>
              <th class="th-left">产品</th>
              <th class="sortable" :class="{sorted:sortKey==='volume'}"      @click="toggleSort('volume')">销量（吨）<i>{{ si('volume') }}</i></th>
              <th>销量环比</th>
              <th class="sortable" :class="{sorted:sortKey==='price'}"       @click="toggleSort('price')">价格<i>{{ si('price') }}</i></th>
              <th>价格环比</th>
              <th class="sortable" :class="{sorted:sortKey==='correlation'}" @click="toggleSort('correlation')">相关系数<i>{{ si('correlation') }}</i></th>
              <th>健康度</th>
            </tr>
          </thead>
        </table>
          ...
          <table class="base-table">
            ...
            <tbody>
              ...
              <tr
                ...
                :class="{ 'row-active': selectedProduct === `${item.productCode}-${item.region}`, 'row-stripe': index % 2 === 1, 'row-poor': healthRating(item).rank >= 2 }"
              ...
              >
                <td class="td-product">
                  <div class="prod-cell">
                    ...
                    <span class="prod-name" :class="{ 'name-active': selectedProduct === `${item.productCode}-${item.region}` }">{{ item.product }}</span>
                  </div>
                </td>
                <td class="td-c"><b>{{ formatNumber(item.latestVolume) }}</b></td>
                <td class="td-c">
                  <span class="pill" :class="cc(item.volumeChange)">
                    ...
                    <span v-else-if="Math.abs(item.volumeChange) < 0.001">—</span>
                    <span v-if="Math.abs(item.volumeChange) >= 0.001">{{ fmtPct(item.volumeChange) }}</span>
                  </span>
                </td>
                <td class="td-c"><b>{{ formatPrice(item) }}</b></td>
                <td class="td-c">
                  <span class="pill ppill" :class="cc(item.priceChange)">
                    <svg v-if="item.priceChange > 0.002"  class="arr" viewBox="0 0 10 10"><path d="M5 1L9 9H1Z" fill="currentColor"/></svg>
                    <svg v-else-if="item.priceChange < -0.002" class="arr" viewBox="0 0 10 10"><path d="M5 9L1 1H9Z" fill="currentColor"/></svg>
                    <span v-else-if="Math.abs(item.priceChange) < 0.001">—</span>
                    <span v-if="Math.abs(item.priceChange) >= 0.001">{{ fmtPct(item.priceChange) }}</span>
                  </span>
                </td>
                <td class="td-c">
                  <span class="corr" :class="item.correlation >= 0 ? 'c-pos' : 'c-neg'">{{ item.correlation > 0 ? '+' : '' }}{{ item.correlation.toFixed(2) }}</span>
                </td>
                <td class="td-c">
                  <span class="hbadge" :class="`hb-${healthRating(item).cls}`">{{ healthRating(item).label }}</span>
                </td>
              </tr>
            </tbody>
          </table>
    ...
    <div class="mobile-only mobile-list">
      ...
      <div
        ...
        class="m-card"
        :class="{ 'mcard-poor': healthRating(item).rank >= 2, 'mcard-selected': selectedProduct === `${item.productCode}-${item.region}` }"
        @click="handleRowClick(item)"
      >
        <!-- 卡片头 -->
        <div class="m-card-hd">
        ...
        <div class="m-card-body">
          <div class="m-metric">
            <span class="m-label">销量</span>
            <span class="m-val">{{ formatNumber(item.latestVolume) }} 吨</span>
          </div>
          <div class="m-metric">
            <span class="m-label">销量环比</span>
            <span class="m-val" :class="cc(item.volumeChange)">{{ Math.abs(item.volumeChange) < 0.001 ? '—' : fmtPct(item.volumeChange) }}</span>
          </div>
          <div class="m-metric">
            <span class="m-label">价格</span>
            <span class="m-val">{{ formatPrice(item) }}</span>
          </div>
          <div class="m-metric">
            <span class="m-label">价格环比</span>
            <span class="m-val" :class="cc(item.priceChange)">{{ Math.abs(item.priceChange) < 0.001 ? '—' : fmtPct(item.priceChange) }}</span>
          </div>
        </div>
        <!-- 卡片尾：相关系数 -->
        <div class="m-card-ft">
          <span class="m-label">相关系数</span>
          <span class="corr" :class="item.correlation >= 0 ? 'c-pos' : 'c-neg'">{{ item.correlation > 0 ? '+' : '' }}{{ item.correlation.toFixed(2) }}</span>
        </div>
      </div>
    </div>
    ...
    <div class="status-bar">
      ...
      <span class="s-hint">点击行切换趋势图</span>
    </div>
...
.banner-tag  { color: #92400e; font-size: var(--fs-xs); flex: 1; min-width: 120px; }
...
thead th { padding: 12px 12px; font-size: var(--fs-sm); font-weight: 600; color: #64748b; background: #f0f6ff; text-align: center; border-bottom: 2px solid #e2e8f0; white-space: nowrap; user-select: none; }
th.th-left { text-align: left; }
th.sortable { cursor: pointer; }
th.sortable:hover { background: #e0ecff; color: #1d4ed8; }
th.sorted { background: #dbeafe; color: #1d4ed8; }
th i { font-style: normal; margin-left: 2px; font-size: var(--fs-xs); opacity: .7; }
.data-row { cursor: pointer; transition: background .1s; }
.data-row td { padding: 0 12px; height: 52px; vertical-align: middle; border-bottom: 1px solid #f1f5f9; }
...
.prod-name { font-weight: 500; color: #374151; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.name-active { font-weight: 700; color: #1d4ed8; }
.td-c { text-align: center; }
b { font-weight: 600; color: #111827; }
.pill { display: inline-flex; align-items: center; gap: 2px; padding: 1px 6px; border-radius: 20px; font-size: var(--fs-xs); font-weight: 600; white-space: nowrap; }
...
.corr { font-weight: 700; font-size: var(--fs-xs); }
.c-pos { color: #1d4ed8; }
.c-neg { color: #dc2626; }
.hbadge { display: inline-block; width: 26px; padding: 2px 0; text-align: center; border-radius: 5px; font-size: var(--fs-xs); font-weight: 700; }
...
/* ── 手机端卡片 ── */
.mobile-list { padding: 0 12px; display: flex; flex-direction: column; gap: 10px; max-height: 450px; overflow-y: auto; }
.m-card { background: #fff; border: 1px solid #e2e8f0; border-radius: 10px; padding: 12px 14px; cursor: pointer; transition: all .15s; }
.m-card:active { transform: scale(0.99); }
.mcard-poor     { background: #fffbeb; border-color: #fde68a; }
.mcard-selected { outline: 2px solid #3b82f6; outline-offset: -1px; background: #eff6ff !important; border-color: #93c5fd !important; }
...
.m-card-body { display: grid; grid-template-columns: 1fr 1fr; gap: 8px 16px; margin-bottom: 8px; }
.m-card-ft { display: flex; align-items: center; gap: 8px; padding-top: 8px; border-top: 1px solid #f1f5f9; }
