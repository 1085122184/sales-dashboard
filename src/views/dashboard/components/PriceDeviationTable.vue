...
watch(() => props.data, (newData) => {
  if (newData && newData.length > 0) {
    const hasAlerts = newData.some(d => getRateLevelCached(d.deviationRate) !== 'normal')
    activeTab.value = hasAlerts ? 'alert' : 'all'
  }
}, { immediate: true })

function getRateLevelCached(rate: number): 'danger' | 'warning' | 'normal' {
...
// 只在 processedData 引用变化时触发（即筛选条件或原始数据变化时）
...
function formatPrice(item: PriceDeviationItem, field: 'sevenDay' | 'today' | 'amount') {
  ...
  else if (field === 'today') rawVal = item.todayAvgPrice
  else if (field === 'amount') rawVal = item.deviationAmount

  // 🌟 核心防线：不管后端传过来的是 null, undefined 还是空字符串，统统安全转为纯数字！
  const val = Number(rawVal) || 0

  const formatOptions = { minimumFractionDigits: 2, maximumFractionDigits: 2 }

  if (field === 'amount') {
    if (val > 0) return `+${symbol}${val.toLocaleString('en-US', formatOptions)}`
    if (val < 0) return `-${symbol}${Math.abs(val).toLocaleString('en-US', formatOptions)}`
    return `${symbol}0.00`
  }
  return `${symbol}${val.toLocaleString('en-US', formatOptions)}`
}
</script>
...
<template>
  <div class="panel">
    ...
    <div v-if="alertSummary.danger > 0 || alertSummary.warning > 0" class="alert-banner">
      ...
      <span v-if="alertSummary.danger  > 0" class="banner-tag danger" >严重 {{ alertSummary.danger }}  个（≤ -20%）</span>
      <span v-if="alertSummary.warning > 0" class="banner-tag warning">警告 {{ alertSummary.warning }} 个（-20%~-10%）</span>
