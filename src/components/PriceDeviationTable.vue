...
// 🌟 修改 1：将默认选中的 Tab 从 'all' 改为 'alert'，默认只看告警数据
...
const processedData = computed(() => {
  ...
  // 🌟 修改 2：移除默认的强制级别排序，仅在用户点击表头时才进行排序
  // 这样当 sortKey 为 'default' 时，完全保留后台 API 返回的原始顺序
...
<template>
  <div class="panel">
    ...
    <div v-if="alertSummary.danger > 0 || alertSummary.warning > 0" class="alert-banner">
      ...
      <span v-if="alertSummary.danger > 0" class="banner-tag danger">严重 {{ alertSummary.danger }} 个（偏差率 ≤ -10%）</span>
    ...
    <div class="toolbar">
      <div class="tab-group">
        <button v-for="tab in (['all','alert','normal'] as Tab[])" :key="tab" class="tab-btn" :class="{ active: activeTab === tab }" @click="activeTab = tab; searchText = ''">
      ...
      <table class="base-table">
        ...
        <thead>
          <tr>
            ...
            <th class="sortable" :class="{sorted: sortKey==='sevenDay'}" @click="toggleSort('sevenDay')">七日均价（元）<i>{{ sortIcon('sevenDay') }}</i></th>
            <th class="sortable" :class="{sorted: sortKey==='today'}" @click="toggleSort('today')">当日均价（元）<i>{{ sortIcon('today') }}</i></th>
          ...
          <tbody>
            <tr v-if="processedData.length === 0">
              <td colspan="5" class="empty-td">
                <div class="empty-box">
                  <svg viewBox="0 0 48 48" fill="none" stroke="#d1d5db" stroke-width="2" style="width:38px;height:38px"><rect x="8" y="8" width="32" height="32" rx="4"/><path d="M16 20h16M16 28h10"/></svg>
                  暂无匹配数据
                </div>
              </td>
            </tr>
            <tr
              ...
              :class="[
                ...
                {
                  'row-selected': selectedProduct === `${item.productCode}-${item.region}`,
            ...
            >
              <td class="td-product">
                <div class="prod-cell">
                  ...
                  <span v-if="getRateLevel(item.deviationRate) !== 'normal'" class="pulse-dot" :class="`pdot-${getRateLevel(item.deviationRate)}`" />
                </div>
              </td>
              <td class="td-num">{{ item.sevenDayAvgPrice.toLocaleString('zh-CN') }}</td>
              <td class="td-num">{{ item.todayAvgPrice.toLocaleString('zh-CN') }}</td>
              <td class="td-num" :class="`txt-${getRateLevel(item.deviationRate)}`">{{ item.deviationAmount.toLocaleString('zh-CN') }}</td>
              <td class="td-rate">
                <span class="rate-pill" :class="`pill-${getRateLevel(item.deviationRate)}`">{{ item.deviationRateText }}</span>
              </td>
            </tr>
          </tbody>
        </table>
...
.banner-tag  { padding: 2px 10px; border-radius: 20px; font-size: var(--fs-xs); font-weight: 600; white-space: nowrap; }
...
.tab-btn { display: flex; align-items: center; gap: 5px; padding: 5px 11px; border: none; border-radius: 6px; background: transparent; color: #64748b; font-size: var(--fs-xs); font-weight: 500; cursor: pointer; font-family: inherit; white-space: nowrap; transition: all .15s; }
...
.s-input { width: 160px; padding: 6px 26px 6px 30px; border: 1px solid #e2e8f0; border-radius: 8px; font-size: var(--fs-xs); color: #374151; background: #f8fafc; outline: none; transition: all .15s; font-family: inherit; }
